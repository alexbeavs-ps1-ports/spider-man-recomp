...
#
# Writes: dist/spider-man-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix spider-man \
  --exe-name Spider_Man_Recompiled \
  --display-name "Spider-Man Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Spider-Man (USA).cue" \
